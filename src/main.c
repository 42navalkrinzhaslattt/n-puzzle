...
//	print_arr(root->arr);
//	printf("%d %d %d %d %d\n", root->heuristic, root->prev_oper, root->depth, root->empty_x, root->empty_y);
	if ((get_inversions(root->arr) + get_manhattan_distance(root->arr, root->empty_y, root->empty_x)) % 2 == 1)//can be optimized to O(n log n);
